...
async function installBullet(): Promise<void> {
    ...
    if (updated) {
        try {
            execSync("cd src/wasm_src/bullet_src && git init && git apply constraint-fix.patch");
        } catch (error: any) {
            console.log(error.output.toString());
            process.exit(1);
        }
        if (fs.existsSync("./src/wasm_src/bullet_src/.git")) {
            fs.rmSync("./src/wasm_src/bullet_src/.git", { recursive: true });
        }
    }
}
