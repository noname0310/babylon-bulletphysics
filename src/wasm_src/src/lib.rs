...
pub fn init() {
    ...
    let world_transform = motion_state.get_world_transform();
    ...
    let world_transform = mation_state_bundle.get_world_transform(0);
    web_sys::console::log_1(&format!("world_transform: {:?}", world_transform).into());
    let world_transform = mation_state_bundle.get_world_transform(1);
    web_sys::console::log_1(&format!("world_transform: {:?}", world_transform).into());
}
