...
/* harmony export */ });
/* harmony import */ var _Maths_sphericalPolynomial_js__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(4640);
/* harmony import */ var _Misc_dds_js__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(3537);
...
class _DDSTextureLoader {
    ...
    loadCubeData(imgs, texture, createPolynomials, onLoad) {
        ...
        if (Array.isArray(imgs)) {
            for (let index = 0; index < imgs.length; index++) {
                ...
                info = _Misc_dds_js__WEBPACK_IMPORTED_MODULE_1__/* .DDSTools */ .D.GetDDSInfo(data);
                texture.width = info.width;
                texture.height = info.height;
                loadMipmap = (info.isRGB || info.isLuminance || info.mipmapCount > 1) && texture.generateMipMaps;
                engine._unpackFlipY(info.isCompressed);
                _Misc_dds_js__WEBPACK_IMPORTED_MODULE_1__/* .DDSTools */ .D.UploadDDSLevels(engine, texture, data, info, loadMipmap, 6, -1, index);
            ...
            loadMipmap = (info.isRGB || info.isLuminance || info.mipmapCount > 1) && texture.generateMipMaps;
            engine._unpackFlipY(info.isCompressed);
            _Misc_dds_js__WEBPACK_IMPORTED_MODULE_1__/* .DDSTools */ .D.UploadDDSLevels(engine, texture, data, info, loadMipmap, 6);
            if (!info.isFourCC && info.mipmapCount === 1) {
    ...
    loadData(data, texture, callback) {
        const info = _Misc_dds_js__WEBPACK_IMPORTED_MODULE_1__/* .DDSTools */ .D.GetDDSInfo(data);
        ...
        callback(info.width, info.height, loadMipmap, info.isFourCC, () => {
            _Misc_dds_js__WEBPACK_IMPORTED_MODULE_1__/* .DDSTools */ .D.UploadDDSLevels(texture.getEngine(), texture, data, info, loadMipmap, 1);
        });
    }
}
